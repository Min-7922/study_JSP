...
<%
	...
	// 변수를 선언했을 때 출력한다면, 분기문에 상관없이 값이 할당되어 있어야 한다 (변수의 초기화)
	...
	String msg = "";
	int answer = 0;
	if((num1 != null | num2 != null) && "".equals(num1) == false && "".equals(num2) == false) {
		answer = Integer.parseInt(num1) + Integer.parseInt(num2);
		msg = String.format("%s + %s = %s", num1, num2, answer);
	}else {
		msg = "정수를 입력하세요.";
	}
	
	
	// 코드의 순서와 상관없이 자바코드와 html코드가 만난다면 우선적으로 java코드가 먼저 실행된다!
	// 따라서 스크립틀릿을 먼저 작성 후에 아래에 html을 작성하는 것이 좋다
%>

<h3>HTML과 JAVA코드가 하나의 페이지에 작성되어 있고, 순서가 섞여도 항상 자바코드부터 수행된다</h3>
<h3>Java코드는 웹 애플리케이션 서버에서 실행되고, HTML코드는 클라이언트의 브라우저에서 실행된다</h3>
