...
<form>
	...
	<!-- form태그의 action 속성이 없으면, [현재 주소]로 새로운 요청을 보낸다 -->
</form>
<%@ page import="java.util.ArrayList, food.FoodDTO" %>
<!-- ArrayList 클래스를 사용하기 위해서 import가 필요하기 떄문에 지시자(directive)를 사용한다 -->

<%!	// 선언부 (declaration, 멤버 필드 및 멤버 메서드를 선언/정의 한다)
	// 지역변수는 소멸하므로 선언부에서 멤버필드로 선언한다.
...
<%	// 스크립틀릿 내부 코드는 service() 함수 내부 내용이다
	...
	// 리스트를 멤버필드로 생성해두면, 객체가 소멸하기 전까지 (= 서버를 재시작하기 전까지) 값이 유지된다
	
	// 4개의 파라미터를 전달받기
	String category = request.getParameter("category");		// <select name = "category">
	String storeName = request.getParameter("storeName");	// <select name = "storeName">
	String menuName = request.getParameter("menuName");		// <select name = "menuName">
	String menuPrice = request.getParameter("menuPrice");	// <select name = "menuPrice">
